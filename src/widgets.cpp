...
void FinancialGrid::populate(const FinancialDashboard& dash, const CloudbedsMetrics& cb)
{
    ...
    cell(row, 0, "Room Revenue - Other", Palette::ROOM, QColor(), true, true);
    cell(row, 1, fmtMoney(cb.room_revenue_othr), Palette::ROOM);
    cell(row, 2, fmtMoney(cb.mtd_revenue_othr), Palette::ROOM);
    cell(row, 3, fmtMoney(cb.ly_mtd_revenue_othr), Palette::ROOM);
    variance = cb.mtd_revenue_othr - cb.ly_mtd_revenue_othr;
    variance_pct = (variance / cb.ly_mtd_revenue_othr) * 100;
    cell(row, 4, fmtMoney(variance), Palette::ROOM, negFg(variance));
    cell(row, 5, fmtVariance(variance_pct), varColor(variance_pct), QColor());

    // naRow(row++, "Room Revenue - Other",   Palette::WHITE);
    row++;
    ...
    // Total row
    double totToday = dash.today.total_amount + cb.room_revenue + cb.room_revenue_othr;
    double totMtd = dash.mtd.total_amount + cb.mtd_revenue + cb.mtd_revenue_othr;
    double totMtdLy = dash.mtd_last_year.total_amount + cb.ly_mtd_revenue + cb.ly_mtd_revenue_othr;
